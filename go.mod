module github.com/E-Health/goscar-export

go 1.14

require (
	github.com/E-Health/goscar v1.2.3
	github.com/boj/redistore v0.0.0-20180917114910-cd5dcc76aeff // indirect
	github.com/corpix/uarand v0.1.1 // indirect
	github.com/gin-gonic/contrib v0.0.0-20191209060500-d6e26eeaa607 // indirect
	github.com/gin-gonic/gin v1.6.3 // indirect
	github.com/google/uuid v1.1.1
	github.com/gorilla/sessions v1.2.0 // indirect
	github.com/icrowley/fake v0.0.0-20180203215853-4178557ae428 // indirect
	github.com/intervention-engine/fhir v0.0.0-20180827153403-cd152e6130c6 // indirect
	github.com/itsjamie/gin-cors v0.0.0-20160420130702-97b4a9da7933 // indirect
	github.com/joho/godotenv v1.3.0
	github.com/jroimartin/gocui v0.4.0
	github.com/juju/errors v0.0.0-20200330140219-3fe23663418f // indirect
	github.com/mattn/go-runewidth v0.0.9 // indirect
	github.com/mitre/heart v0.0.0-20160825192324-0c46b433a490 // indirect
	github.com/nsf/termbox-go v0.0.0-20200418040025-38ba6e5628f1 // indirect
	github.com/samply/golang-fhir-models/fhir-models v0.1.0
	golang.org/x/oauth2 v0.0.0-20200107190931-bf48bf16ab8d // indirect
	gopkg.in/mgo.v2 v2.0.0-20190816093944-a6b53ec6cb22 // indirect
	gopkg.in/square/go-jose.v1 v1.1.2 // indirect
)
